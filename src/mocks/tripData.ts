import type { AnalysisStatus, DuplicateStatus, ReceiptItem } from "../features/receipt/types/receipt";
...
export function getTripMembers(trip: Trip): Member[] {
  return trip.memberIds.map((id) => getMember(id)).filter((member): member is Member => member !== undefined);
}
