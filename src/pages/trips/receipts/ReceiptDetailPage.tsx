...
function ReceiptDetailPage() {
  ...
  const updateDraftItem = (id: string, field: "name" | "quantity" | "amount", value: string) => {
    setDraftItems((items) =>
      items.map((item) =>
        item.id === id ? { ...item, [field]: field === "name" ? value : Math.max(0, Number(value)) } : item,
      ),
    );
  };
  ...
  const addDraftItem = () => {
    ...
    setDraftItems((items) => [...items, { id: nextId, name: "", quantity: 1, amount: 0, confidence: "HIGH" }]);
  };
    ...
    <div className="receipt-detail-page">
      <header className="topbar">
        <div className="title-group">
          <button className="back-link" type="button" onClick={() => navigate(routePaths.tripHome(tripId))}>
            <Icon name="arrow" />
            여행 홈
          </button>
          <div className="title-copy">
            ...
            <p>
              {formatDateTime(receipt.paidAt)} · {receipt.payerName}님 결제
            </p>
          </div>
        </div>
        <div className="header-actions">
          ...
          <button
            ...
            type="button"
            onClick={() => temporaryAction("삭제는 아직 서버에 반영되지 않습니다.", "danger")}
          >
            <Icon name="trash" />
            삭제
          </button>
          <button
            ...
            type="button"
            onClick={() => navigate(routePaths.receiptSplit(tripId, receiptId))}
          >
        ...
        <section className="receipt-column" aria-labelledby="original-title">
          <div className="section-heading">
            <div>
              ...
              <h2 id="original-title">원본 영수증</h2>
            </div>
          </div>
          ...
          <div className="summary-card">
            <div className="summary-title">
              <h2>결제 정보</h2>
              <button className="text-button" type="button" onClick={openInfoEditor}>
                <Icon name="edit" />
                정보 수정
              </button>
            </div>
            <dl>
              ...
              <div className="amount-row">
                <dt>총액</dt>
                <dd>{won.format(receipt.totalAmount)}</dd>
              </div>
              <div>
                <dt>등록·결제자</dt>
        ...
        <section className="analysis-column" aria-labelledby="items-title">
          <div className="section-heading items-heading">
            <div>
              ...
              <h2 id="items-title">
                인식된 상품 <em>{receipt.items.length}</em>
              </h2>
            </div>
            <button className="button button-outline" type="button" onClick={openItemEditor}>
              <Icon name="edit" />
              항목 추가·수정
            </button>
          </div>
          ...
          <div className="items-table" role="table" aria-label="OCR 인식 상품 목록">
            <div className="table-row table-head" role="row">
              ...
              <span role="columnheader">신뢰도</span>
            </div>
            {receipt.items.map((item) => (
              <div
                className={`table-row ${item.confidence === "LOW" ? "low-confidence" : ""}`}
                role="row"
                key={item.id}
              >
                <div className="product-cell" role="cell">
                  <strong>{item.name}</strong>
                  {item.confidence === "LOW" && <small>인식 불확실 — 확인 필요</small>}
                </div>
          ...
          <section className={`validation-card ${totalMatches ? "is-valid" : "needs-check"}`}>
            <div className="validation-icon">
              <Icon name={totalMatches ? "check" : "receipt"} />
            </div>
            <div className="validation-copy">
              <div className="validation-title">
                <h3>총액 검증</h3>
                <span>{totalMatches ? "일치" : "확인 필요"}</span>
              </div>
              <p className="calculation">
                상품 합계 <strong>{won.format(itemTotal)}</strong> <b>{totalMatches ? "=" : "≠"}</b> 결제 총액{" "}
                <strong>{won.format(receipt.totalAmount)}</strong>
              </p>
              <p className="helper">
                {totalMatches
                  ? "인식된 상품의 합계와 결제 총액이 일치합니다."
                  : "할인, 쿠폰, 배달비 등으로 상품 합계와 결제 총액이 다를 수 있습니다. 확인이 필요합니다."}
              </p>
            </div>
          </section>

          <section className="discount-row">
            <div>
              <h3>할인·쿠폰 반영</h3>
              <p>할인은 별도 항목으로 나누지 않고 최종 총액에 반영됩니다.</p>
            </div>
            <span>해당 없음</span>
          </section>
          ...
          {receipt.duplicateStatus !== "CLEAR" && (
            <section className="duplicate-card">
              <div className="duplicate-content">
                ...
                <div>
                  <h3>중복 영수증이 의심돼요</h3>
                  <p>비슷한 시간과 금액의 영수증이 있습니다. 자동으로 삭제하지 않으니 등록 여부를 선택해 주세요.</p>
                </div>
              </div>
              <div className="duplicate-actions">
                <button
                  className={`button ${duplicateChoice === "keep" ? "button-primary" : "button-outline"}`}
                  type="button"
                  onClick={() => {
                    setDuplicateChoice("keep");
                    temporaryAction("계속 등록을 선택했습니다. 서버에는 저장되지 않습니다.");
                  }}
                >
                  계속 등록
                </button>
                <button
                  className={`button ${duplicateChoice === "cancel" ? "button-cancel-active" : "button-quiet"}`}
                  type="button"
                  onClick={() => {
                    setDuplicateChoice("cancel");
                    temporaryAction("등록 취소를 선택했습니다. 서버에는 반영되지 않습니다.", "danger");
                  }}
                >
                  등록 취소
                </button>
      ...
      <Modal
        isOpen={infoOpen}
        title="결제 정보 수정"
        description="변경 내용은 현재 화면에만 반영됩니다."
        ...
        <form onSubmit={saveInfo}>
          <div className="form-grid">
            <label>
              ...
              <input
                required
                value={draftInfo.merchantName}
                onChange={(e) => setDraftInfo({ ...draftInfo, merchantName: e.target.value })}
              />
            </label>
            <label>
              <span>결제일시</span>
              <input
                required
                value={draftInfo.paidAt}
                onChange={(e) => setDraftInfo({ ...draftInfo, paidAt: e.target.value })}
              />
            </label>
            <label>
              <span>총액</span>
              <div className="input-with-unit">
                <input
                  ...
                  min="0"
                  type="number"
                  value={draftInfo.totalAmount}
                  onChange={(e) => setDraftInfo({ ...draftInfo, totalAmount: Number(e.target.value) })}
                />
                <span>원</span>
              </div>
            </label>
          ...
          <div className="modal-actions">
            <button className="button button-quiet" type="button" onClick={() => setInfoOpen(false)}>
              취소
            </button>
      ...
      <Modal
        isOpen={itemsOpen}
        title="상품 항목 수정"
        description="상품명, 수량과 상품별 금액을 확인해 주세요."
      ...
      >
        <form onSubmit={saveItems}>
          ...
          <button className="add-item-button" type="button" onClick={addDraftItem}>
            <Icon name="plus" />새 항목 추가
          </button>
          <div className="modal-actions">
            <button className="button button-quiet" type="button" onClick={() => setItemsOpen(false)}>
              취소
            </button>
            <button className="button button-primary" type="submit">
              목록에 적용
